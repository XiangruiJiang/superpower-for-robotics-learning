...
```
✅ Short GPU train/rollout → inspect reward / episode / termination / NaN logs → play from relevant checkpoint → report physical quantities
❌ "Code looks right" / "One old checkpoint looked okay"
```

## RL / Simulation Projects

When project behavior depends on learning dynamics, reward shaping, or physics, the honest proof is usually runtime evidence rather than static inspection. Use the smallest GPU training or rollout command that can reproduce the claim, inspect the resulting logs, then run `play.py` (or the project equivalent) on a relevant checkpoint and report the physical quantities or behaviors you observed.
